...
INSERT INTO updatescripts (`id`,`majorVersion`,`minorVersion`,`buildVersion`,`script`,`description`) VALUES (74,1,0,24,'CREATE TABLE %SCHEMA%.`linkuserterminalconfigsjobs` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `configId` INT NOT NULL DEFAULT 1,
  `jobId` INT NOT NULL DEFAULT 1,
  PRIMARY KEY (`id`, `jobId`, `configId`),
  INDEX `fk_lutcj_config_idx` (`configId` ASC) VISIBLE,
  INDEX `fk_lutcj_job_idx` (`jobId` ASC) VISIBLE,
  CONSTRAINT `fk_lutcj_config`
    FOREIGN KEY (`configId`)
    REFERENCES %SCHEMA%.`userterminalconfigs` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_lutcj_job`
    FOREIGN KEY (`jobId`)
    REFERENCES %SCHEMA%.`jobs` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE);
','Link config jobs');
INSERT INTO updatescripts (`id`,`majorVersion`,`minorVersion`,`buildVersion`,`script`,`description`) VALUES (75,1,0,23,'ALTER TABLE %SCHEMA%.`translations` 
ADD COLUMN `lastmodified` TIMESTAMP NULL AFTER `enabled`;
','Last modified for translations');
INSERT INTO updatescripts (`id`,`majorVersion`,`minorVersion`,`buildVersion`,`script`,`description`) VALUES (76,1,0,25,'use %SCHEMA%;

ALTER TABLE bomitems 
ADD COLUMN `showOnTerminal` TINYINT(1) NULL DEFAULT '1' AFTER `enabled`;

ALTER TABLE cells 
ADD COLUMN `showOnTerminal` TINYINT(1) NULL DEFAULT '1' AFTER `enabled`;

ALTER TABLE celltypes 
ADD COLUMN `showOnTerminal` TINYINT(1) NULL DEFAULT '1' AFTER `enabled`;

ALTER TABLE intermissions 
ADD COLUMN `showOnTerminal` TINYINT(1) NULL DEFAULT '1' AFTER `enabled`;

ALTER TABLE jobinputtypes 
ADD COLUMN `showOnTerminal` TINYINT(1) NULL DEFAULT '1' AFTER `enabled`;

ALTER TABLE joboperations 
ADD COLUMN `showOnTerminal` TINYINT(1) NULL DEFAULT '1' AFTER `enabled`;
...
ALTER TABLE objectdocumenttypes 
ADD COLUMN `showOnTerminal` TINYINT(1) NULL DEFAULT '1' AFTER `enabled`;
...
','ShowOnTerminal field');
INSERT INTO updatescripts (`id`,`majorVersion`,`minorVersion`,`buildVersion`,`script`,`description`) VALUES (77,1,0,25,'use ww;','Test 1');
INSERT INTO updatescripts (`id`,`majorVersion`,`minorVersion`,`buildVersion`,`script`,`description`) VALUES (78,1,0,26,'select * from updatescripts;','Test 2');
